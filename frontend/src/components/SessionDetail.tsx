...
export function SessionDetail() {
  ...
  return (
    <section className="max-w-7xl mx-auto px-6 py-12 animate-in fade-in duration-500">
      ...
      <button
        onClick={() => navigate("/")}
        className="flex items-center gap-2 font-bold text-white hover:text-sun-yellow mb-6 transition-colors group"
      >
      ...
      <div className="mb-12">
        <div className="flex flex-col md:flex-row justify-between items-end mb-6 gap-4">
          <div>
            <h1 className="text-3xl md:text-4xl font-display font-bold text-kingfisher-midnight">
              Recording Session
            </h1>
            <div className="flex items-center gap-2 text-slate-500 mt-2">
              <Calendar className="w-4 h-4 text-kingfisher-sky" />
              <span className='font-bold text-kingfisher-midnight'>{new Date(startTime).toLocaleString()}</span>
              <span className="mx-2 ">•</span>
              <Badge variant="neutral">
                {sessionDetections.length} Detections
              </Badge>
            </div>
          </div>
        </div>
        ...
        {sessionDetections.length > 0 && sessionDetections[0].audio_url && (
          <div className="mt-6 bg-slate-100 rounded-xl p-6 border border-slate-200">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h4 className="font-bold text-kingfisher-midnight">
                  Full Recording
                </h4>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {sessionDetections.map((bird, idx) => (
            <Card
              key={bird.id || idx}
              className="flex flex-col group bg-white hover:border-kingfisher-sky/30 transition-all border-2 border-transparent"
            >
              <div
              ...
              >
                ...
                <div className="absolute top-2 right-2">
                  <Badge
                    variant={bird.confidence > 0.8 ? "success" : "warning"}
                  >
              ...
              <div className="flex-1">
                <h5 className="text-lg font-bold text-kingfisher-midnight">
                  {bird.species}
                </h5>
          ...
          >
            ...
            <div className="w-full md:w-1/3 bg-white p-8 flex flex-col overflow-y-auto border-l border-slate-100">
              ...
              <div className="hidden md:flex justify-end mb-4">
                <button
                  onClick={() => setSelectedBird(null)}
                  className="p-2 hover:bg-slate-100 text-slate-400 hover:text-kingfisher-midnight rounded-full transition-colors"
                >
                  <X className="w-6 h-6" />
                </button>
              </div>
              ...
              <div className="flex-1">
                <div className="mb-6">
                  <div className="text-sm font-bold text-kingfisher-sky uppercase tracking-wider mb-2">
                    Species Identification
                  </div>
                  <h2 className="text-3xl font-display font-bold text-kingfisher-midnight leading-tight">
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span
                        className={`font-bold text-lg ${selectedBird.confidence > 0.8 ? "text-green-600" : "text-amber-600"}`}
                      >
                  ...
                  <div>
                    ...
                    {/* Location */}
                    {selectedBird.lat !== null &&
                      selectedBird.lon !== null ? (
